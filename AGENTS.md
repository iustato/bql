...
- **`not in`/`not like` собираются из двух токенов по позиции** — тем же приёмом,
  что и знак числа: автомат режет по пробелам и всегда отдаёт `not` идентификатором,
  а приставку опознаёт `toReversePolishNotation()` по флагу «ждём операнд». Поэтому
  имя `not` остаётся свободным для переменной (`not not in ['USA']` разбирается).
  Составные имена всё же зарегистрированы (`NEGATED_OPERATORS`), как и `u-`/`u+`:
  запись в реестре несёт приоритет и число операндов, без неё `getPrecedence()`
  вернул бы 0, а `evaluateRPNToToken()` — «operator is not defined».
  Считаются один раз в `executeOperator()` как `!` поверх базового оператора, а не
  в каждом обработчике значения: базовые `in`/`like` всюду возвращают
  `BoolVarHandler`, поэтому отрицание не может разойтись с исходным оператором.
- **Два значения подряд — ошибка** (`toReversePolishNotation()`). Лишний токен
  уезжал в стек ОПН, откуда его вытеснял результат, поэтому `x not in [...]`
  до появления составного оператора молча считалось как `null in [...]`. Флаг
  «ждём операнд» заодно различает префиксный `--x` и постфиксный `a++`.
